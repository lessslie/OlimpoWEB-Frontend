...
const UserDetailPage = ({ params }: { params: { id: string } }) => {
  ...
  const fetchAttendances = async () => {
    try {
      ...
      const sortedAttendances = [...mockAttendances].sort((a, b) => 
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
              ...
              <div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1">{userData.phone || 'No especificado'}</p>
                  </div>
                </div>
              </div>
              <div>
            ...
            <div className="pt-6 border-t border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <p className="mt-1 text-2xl font-semibold">{attendances.length}</p>
                </div>
                <div className="bg-gray-50 p-4 rounded-lg">
                  <p className="text-sm font-medium text-gray-500">Última asistencia</p>
                  <p className="mt-1 text-sm">
                    {attendances.length > 0 
                      ? formatDate(attendances[attendances.length - 1].check_in_time)
                      : 'Sin asistencias'}
                  </p>
                </div>
              </div>
        ...
        {activeTab === 'memberships' && (
          <div>
            <div className="flex justify-between items-center mb-6">
              ...
              <button
                onClick={() => router.push(`/admin/memberships/new?userId=${params.id}`)}
                    ...
                    {memberships.map((membership) => (
                      <tr key={membership.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-medium text-gray-900">{membership.type}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getMembershipStatusColor(membership.status)}`}>
        ...
        {activeTab === 'attendance' && (
          <div>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-medium text-gray-900">Historial de asistencias</h3>
              <div>
            ...
            {attendances.length > 0 ? (
              <>
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {showAllAttendances 
                        ? attendances
                            .slice((currentPage - 1) * attendancesPerPage, currentPage * attendancesPerPage)
                            .map((attendance) => (
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                  <div className="text-sm text-gray-900">
                                    {attendance.membership_id 
                                      ? memberships.find(m => m.id === attendance.membership_id)?.type || 'No disponible'
                                      : 'No asociada'
                                    }
                                  </div>
                                </td>
                ...
                {/* Paginación */}
                {showAllAttendances && attendances.length > attendancesPerPage && (
                  <div className="flex justify-center mt-6">
                    <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                      ...
                      {/* Números de página */}
                      {Array.from({ length: Math.ceil(attendances.length / attendancesPerPage) }).map((_, index) => (
                        <button
                          key={index}
                      ...
                      <button
                        onClick={() => setCurrentPage(prev => Math.min(prev + 1, Math.ceil(attendances.length / attendancesPerPage)))}
                        disabled={currentPage === Math.ceil(attendances.length / attendancesPerPage)}
                        className={`relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium ${
                          currentPage === Math.ceil(attendances.length / attendancesPerPage)
                            ? 'text-gray-300 cursor-not-allowed'
                            : 'text-gray-500 hover:bg-gray-50'
                        }`}
                      >
        ...
        {activeTab === 'routine' && (
          <div>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-medium text-gray-900">Rutina de entrenamiento</h3>
            ...
            {hasRoutine ? (
              ...
              <div className="py-8 text-center text-gray-500">
                <p className="mb-4">Este usuario no tiene una rutina asignada.</p>
                <p>Puedes asignarle una rutina personalizada haciendo clic en el botón "Asignar rutina".</p>
              </div>
            )}
          </div>
        )}
